...
Result<Void> Server::init() {
    ...
    for (std::map<unsigned int, ServerConfig>::const_iterator it =
        ...
        int opt = 1;
        TRY_(Void, Void,
             server_fd.set_socket_option(SOL_SOCKET, SO_REUSEADDR, &opt,
                                         sizeof(opt)))
...
Result<Void> Server::start(char** envp) {
    ...
    while (g_receivedSignal == 0) {
        ...
        std::vector<std::pair<const FileDescriptor*, Response> >
            clients_to_flush;
        ...
        for (std::map<const FileDescriptor*, ClientSession>::iterator it =
            ...
            if (timeout_ms > 0) {
                const long long elapsed_ms =
                    ...
                    (now.tv_nsec - session.last_activity_time.tv_nsec) /
                        1000000LL;
                if (elapsed_ms < 0) {
                    std::cerr << utils::error
                              << "client activity time in the future"
                              << std::endl;
                    continue;
                }
        ...
        for (size_t i = 0; i < clients_to_flush.size(); ++i) {
            ...
            std::map<FileDescriptor const*, ClientSession>::iterator jt =
                clients.find(fd);
            if (jt == clients.end()) continue;
            std::ostringstream oss;
            oss << resp;
            if (!resp.keep_alive) jt->second.dropping = true;
        ...
        for (std::map<const FileDescriptor*,
            ...
            if (cgi->check_timeout()) {
                ...
                if (resp_res.has_value()) {
                    std::map<FileDescriptor const*, ClientSession>::iterator
                        jt = clients.find(it->second.first);
                    if (jt == clients.end()) {
                        cgis_to_reap.insert(cgi);
                        continue;
                    }
                    const Result<Response> resp_res2 =
                        Response::from_cgi_outbuff(
                            resp_res.value(), jt->second.config->get_header());
                    resp = resp_res2.has_value()
                             ? resp_res2.value()
                             : DefaultError::default_err_response(
                                   Response::BAD_GATEWAY);
                } else {
                    resp = DefaultError::default_err_response(
                        Response::BAD_GATEWAY);
                }
        ...
        Result<Events> events_result =
            epoll.wait(static_cast<int>(epoll_timeout));
        if (!events_result.has_value()) {
            if (events_result.error() == Errors::interrupted) continue;
        ...
        for (Events events = events_result.value(); !events.is_end();
            ...
            std::cerr << utils::debug << "epoll event on fd=" << fd->_fd
                      << std::endl;
            if (listeners.find(fd) != listeners.end()) {
                TRY_(Void, Void, new_connection(fd))
            } else if (clients.find(fd) != clients.end()) {
                if (event->in) TRY_(Void, Void, client_read(fd, envp))
                if (event->out) TRY_(Void, Void, client_write(fd))
                ...
                std::map<FileDescriptor const*,
                         std::pair<const FileDescriptor*,
                                   CgiDelegate*> >::iterator it = cgis.find(fd);
                if (it != cgis.end()) {
