...
Result<Void> Server::client_read(const FileDescriptor* client_fd, char** envp) {
    if (clients.find(client_fd) == clients.end()) { return OKV; }
    ...
    while (true) { // repeat until nothing to read
        ...
        if (!recv_res.has_value()) break; // EWOULDBLOCK
        ...
        if (client.req == NULL) {
            ...
            if (!req_.has_value()) {
                std::cerr << utils::error
                          << "request parsing failed: " << req_.error()
                          << std::endl;
                if (req_.error() == Errors::incomplete_header) {
                    if (client.dropping) disconnect(client_fd);
                    return OKV;
                    ...
                    resp.print_simple(std::cout);
                    {
                        ...
                        if (!qr.has_value()) return ERR(Void, qr.error());
                    }
                    return OKV;
            ...
            if (!dis_res.has_value()) std::cerr << dis_res.error() << std::endl;
            // Client may have been disconnected in dispatch_request, check
            // existence
            if (clients.find(client_fd) == clients.end()) return OKV;
            continue;
        } else {
            ...
            if (clients.find(client_fd) == clients.end()) return OKV;
            const Result<Void> parse_more =
                client.req->continue_parsing(in_buffer);
            if (!parse_more.has_value()) {
                std::cerr << utils::error << "request body parsing failed: "
                          << parse_more.error() << std::endl;
                Response resp(
            ...
            if (client.req->is_partial()) return OKV;
...
Result<Void> Server::client_write(const FileDescriptor* client_fd) {
    ...
    if (!write_buffer.empty()) {
        while (true) { // ET 모드이므로 보낼 수 있는 만큼 다 보냄
            Result<ssize_t> send_res = client_fd->sock_send(
                write_buffer.c_str(), write_buffer.length());
            if (!send_res.has_value()) break; // EWOULDBLOCK
    ...
    if (infile.fail()) {
        std::cerr << utils::error << "Stream error: Failed to seek to offset "
                  << client.out_file_offset << " for file "
    ...
    while (client.streaming_file && write_buffer.empty()) {
        ...
        if (read_bytes > 0) {
            ...
            while (!client.out_buff.empty()) {
                ...
                if (!send_res.has_value()) {
                    std::cout << utils::info
                              << "stream: send would block, remaining="
        ...
        // If buffer is not empty after send, break to wait for next EPOLLOUT
        // event
        if (!write_buffer.empty()) break;
    }
    return OKV;
}
