    ...
    if (request->get_path() == rule->path.to_string() &&
        ...
        if (file.is_open()) {
            ...
            if (is_authenticated) {
                ...
                response.cookie = "session_id=" + session_result.value() +
                                  "; Path=/; HttpOnly";
                return response;
            } else {
            ...
            return ResponseErrors::error_response(config, rule,
                                                  Response::NOT_FOUND, envp);
    }

    std::cout << utils::info << "matched rule path: '" << rule->path
              << "' upload_dir = '" << rule->root.to_string() << "'"
              << std::endl;
    if (!rule->root.to_string().empty()) {
        ...
        if (body.length() > static_cast<size_t>(max_body_KB) * 1024) {
            std::cout << utils::warning << "Upload rejected: body size "
                      ...
                      << (max_body_KB * 1024) << std::endl;
            return ResponseErrors::error_response(
                config, rule, Response::PAYLOAD_TOO_LARGE, envp);
        }
        ...
        if (stat(upload_path.c_str(), &st) != 0) {
            ...
            if (errno == EACCES) {
                return ResponseErrors::error_response(
                    config, rule, Response::FORBIDDEN, envp);
            }
            return ResponseErrors::error_response(
                config, rule, Response::INTERNAL_SERVER_ERR, envp);
        }
        if (!S_ISDIR(st.st_mode)) {
            std::cerr << utils::warning
                      << "upload path exists yet not a directory" << std::endl;
            return ResponseErrors::error_response(
                config, rule, Response::INTERNAL_SERVER_ERR, envp);
        }
        if (access(upload_path.c_str(), W_OK) != 0) {
            std::cerr << utils::warning << "upload dir not writable"
                      << std::endl;
            if (errno == EACCES) {
                return ResponseErrors::error_response(
                    config, rule, Response::FORBIDDEN, envp);
        ...
        while (true) {
            ...
            if (filename.empty() && fieldname.empty()) break;
            ...
            if (!filename.empty()) {
                ...
                std::cout << utils::info << "Uploading file: " << file_path
                          << " (size: " << part_data.length() << ")"
                          << std::endl;
                std::cout << utils::info << "First 20 bytes (hex): ";
                for (size_t i = 0;
                     i < std::min(static_cast<size_t>(20), part_data.length());
                     i++) {
...
Response ResponseHandlers::get_method_response(Target target, Response response,
    ...
    if (rule->op == REDIRECT) {
        ...
        Result<std::string> rewritten = config->get_rewritten_path(
    ...
    if (target.type == IS_DIR && rule->op == AUTOINDEX) {
        ...
        if (dir == NULL) {
            if (errno == EACCES) {
                ...
                return ResponseErrors::error_response(
                    config, rule, Response::NOT_FOUND, envp);
            } else {
                return ResponseErrors::error_response(
                    config, rule, Response::INTERNAL_SERVER_ERR, envp);
            }
        ...
        Result<int> path_type_result =
            ResponseUtils::check_path_type(target.path);
        if (!path_type_result.has_value() ||
            path_type_result.value() != IS_FILE)
            return ResponseErrors::error_response(config, rule,
                                                  Response::NOT_FOUND, envp);
...
Response ResponseHandlers::http_response(
    ...
    const ServerConfig* config = client->config;
    if (config == NULL)
        return DefaultError::default_err_response(
            Response::INTERNAL_SERVER_ERR);
    ...
    if (!session_id.empty()) user_session = session->get_session(session_id);
    if (user_session) {
        std::cout << utils::info
        ...
        if (request->get_method() == Request::DELETE) {
            ...
            response = ResponseErrors::error_response(
                config, rule, Response::UNAUTHORIZED, envp);
            response.headers = config->get_header();
            return response;
        } else {
            std::cout << utils::info
    ...
    if (!user_session && !rule->auth_info.empty()) {
        std::cerr << utils::error << "[Authentication] No valid session."
                  << std::endl;
    ...
    if (request->get_path() == "/api/session-info") {
        if (request->get_method() == Request::GET) {
            ...
            if (session_id.empty()) {
                ...
                if (session->get_session_info(session_id, timeout_seconds,
